...
class PrefixSampler:
    ...
    def _sample_from_model(self, texts, min_words=55, truncate_ratio=0.5):
        ...
        if self.args.dataset == 'pubmed':
            ...
            num_untruncate_tokens = max([len(t) * (1.0 - truncate_ratio) for t in texts])
        ...
        tries = 0
        m = 0
        while m < min_words and tries < 10:
            if tries != 0:
                print()
            ...
            outputs = self.base_model.generate(**all_encoded, 
                                               min_new_tokens=min(max(min_words, min_length), int(0.8*num_untruncate_tokens)), 
                                               max_new_tokens=int(1.2*num_untruncate_tokens), 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base_model_name', type=str, default="falcon-7b-instruct")
