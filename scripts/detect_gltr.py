...
def get_rank(logits, labels):
    ...
    ranks = -(ranks.float() + 1).mean() # convert to 1-indexed rank
    ranks = torch.nan_to_num(ranks, nan=-1e4)
...
def get_entropy(logits, labels):
    ...
    entropy = torch.nan_to_num(entropy, nan=-1e4)
